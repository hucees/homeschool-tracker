  ...
  return (
    <AppShell organizationName={organization.name}>
      <div className="grid gap-6">
        <div><Link href="/dashboard/students" className="text-sm font-bold text-[#23685a] hover:underline">← Back to students</Link></div>
        ...
        <section className="app-surface rounded-[26px] p-5 sm:p-8">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-2xl bg-[#f6faf8] p-4"><div className="text-xs font-bold uppercase tracking-wide text-[#718087]">Official placement</div><div className="mt-2 font-bold">{grade?.name ?? "No active grade placement"}</div><div className="mt-1 text-sm text-[#617078]">{academicYear?.name ?? "No active academic year"}</div></div>
            ...
            <div className="rounded-2xl bg-[#f6faf8] p-4 sm:col-span-2 lg:col-span-1"><div className="text-xs font-bold uppercase tracking-wide text-[#718087]">Academic records</div><div className="mt-2 font-bold">{gradeCount ?? 0} current grade(s)</div><div className="mt-1 text-sm text-[#617078]">{assignmentCount ?? 0} assessment(s) · {reportCount ?? 0} official report(s)</div></div>
          </div>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <div className="app-surface rounded-2xl p-5 sm:p-6">
            <div className="flex h-full flex-col justify-between gap-5">
              <div><h2 className="text-xl font-bold">Student portal access</h2><p className="mt-1 text-sm text-[#617078]">School code: <strong>{organization.slug}</strong></p></div>
          ...
          <div className="app-surface rounded-2xl p-5 sm:p-6"><div className="flex h-full flex-col justify-between gap-5"><div><h2 className="text-xl font-bold">Assignments & grades</h2><p className="mt-1 text-sm text-[#617078]">Assign assessments, enter grades, and review competency mastery.</p></div><Link href={`/dashboard/students/${student.id}/gradebook`} className="w-fit rounded-xl bg-[#23685a] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#174d43]">Open gradebook</Link></div></div>

          <div className="app-surface rounded-2xl p-5 sm:p-6 md:col-span-2 xl:col-span-1"><div className="flex h-full flex-col justify-between gap-5"><div><h2 className="text-xl font-bold">Progress & reports</h2><p className="mt-1 text-sm text-[#617078]">Review course progress, attendance, competency status, and permanent report snapshots.</p></div><Link href={`/dashboard/students/${student.id}/progress`} className="w-fit rounded-xl bg-[#456f91] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#345f80]">View progress</Link></div></div>
        </section>

        <section className="app-surface rounded-2xl p-5 sm:p-6">
          <div><div className="text-xs font-bold uppercase tracking-[0.16em] text-[#456f91]">Subject placement</div><h2 className="mt-1 text-xl font-bold">Courses & progression</h2><p className="mt-1 text-sm text-[#617078]">Course advancement is independent from the student&apos;s official grade placement.</p></div>
            ...
            {enrollments?.length ? enrollments.map((enrollment) => {
              ...
              return (
                <div key={enrollment.id} className="flex flex-col justify-between gap-4 rounded-2xl border border-[#dfe7e3] bg-[#fbfcfb] p-4 sm:flex-row sm:items-center">
                  <div><div className="font-bold">{course?.title ?? "Course"}</div><div className="mt-1 text-sm text-[#617078]">{course?.course_code} · Attempt {enrollment.attempt_number} · Started {enrollment.start_date}</div><div className="mt-2"><StatusPill tone={enrollment.status === "completed" ? "green" : enrollment.status === "active" ? "blue" : "gray"}>{enrollment.status}</StatusPill></div></div>
