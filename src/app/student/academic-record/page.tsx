...
export default async function StudentAcademicRecordPage() {
  ...
  const [snapshot, { data: transcriptData }] = await Promise.all([
    buildStudentTranscriptSnapshot({
      supabase,
      organization,
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[#617078]">
              Only an instructor can issue an official transcript. You can view and save any transcript that has already been issued to you.
            </p>
          </div>
          <StatusPill tone="blue">{transcripts.length} available</StatusPill>
        </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {transcripts.length ? (
            ...
            <div className="rounded-xl bg-[#f8faf8] p-4 text-sm text-[#617078] sm:col-span-2">
              No official transcript has been issued yet. Your live academic record above is still available anytime.
            </div>
          )}
        </div>
      </section>
